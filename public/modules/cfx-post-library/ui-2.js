'use strict';
  ...
  function openFolderManager(folderId=null,category=null){
    ...
    const folder=isUngrouped?null:folderById(folderId);const isNew=!folder&&!isUngrouped;
    $('#folderModalTitle').textContent=isUngrouped?`整理未分组条目 · ${state.editingUngroupedCategory}`:isNew?'新建文件夹':`管理文件夹 · ${folder.name}`;
    const opts=ordinaryCategories();$('#folderCategorySelect').innerHTML=opts.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');
    const preferred=isUngrouped?state.editingUngroupedCategory:(folder?.category||(!systemCategories.includes(category||state.filterCategory)?(category||state.filterCategory):'截面与定位器'));$('#folderCategorySelect').value=opts.includes(preferred)?preferred:(opts[0]||'未分类');
    $('#folderNameInput').value=folder?.name||'';$('#folderNameInput').placeholder=isUngrouped?'输入正式文件夹名称，例如：进口流量公式':'文件夹名称';
    $('#folderBatchArea').classList.toggle('hidden',!isNew);$('#folderCopyArea').classList.toggle('hidden',isNew||isUngrouped);$('#folderCopyName').value=folder?`${folder.name} - 副本`:'';$('#folderReplaceFrom').value=folder?.name||'';$('#folderReplaceTo').value='';
  ...
  function saveFolderEdit(){
    const isUngrouped=state.editingFolderId==='__ungrouped__';const old=isUngrouped?null:folderById(state.editingFolderId);const name=cleanFolderName($('#folderNameInput').value),category=cleanCategoryName($('#folderCategorySelect').value);
    if(!name){toast(isUngrouped?'请填写整理后的文件夹名称':'文件夹名称不能为空');return;}if(!category){toast('请选择所属分类');return;}if(name==='未分组'){toast('“未分组”是系统临时分组，请使用其他正式文件夹名称');return;}
    const duplicate=state.folders.find(f=>f.category===category&&f.name===name&&f.id!==old?.id);if(duplicate){toast('该分类中已存在同名文件夹');return;}
    ensureCategory(category);
    if(isUngrouped){
      const sourceCategory=state.editingUngroupedCategory;const items=state.items.filter(x=>x.category===sourceCategory&&!x.folderId);if(!items.length){toast('当前没有可整理的未分组条目');closeModal('folderModal');renderAll();return;}
      const newFolder=normalizeFolder({name,category});state.folders.push(newFolder);items.forEach(x=>{x.category=category;x.folderId=newFolder.id;x.updatedAt=now();});state.collapsedCategories.delete(category);saveCollapsed();state.filterCategory=category;state.filterFolderId=newFolder.id;save();closeModal('folderModal');renderAll();toast(`已将 ${items.length} 个条目整理到“${name}”`);return;
    }
    if(old){const oldCategory=old.category;old.name=name;old.category=category;old.updatedAt=now();state.items.forEach(x=>{if(x.folderId===old.id){x.category=category;x.updatedAt=now();}});if(state.filterFolderId===old.id)state.filterCategory=category;}else state.folders.push(normalizeFolder({name,category}));state.collapsedCategories.delete(category);saveCollapsed();save();closeModal('folderModal');renderAll();toast(old?'文件夹已更新':'文件夹已创建');
  }
