'use strict';
  els.nav.addEventListener('click',e=>{const toggle=e.target.closest('button[data-toggle-cat]');if(toggle){const cat=toggle.dataset.toggleCat;if(state.collapsedCategories.has(cat))state.collapsedCategories.delete(cat);else state.collapsedCategories.add(cat);saveCollapsed();renderNav();return;}const folderManage=e.target.closest('button[data-manage-folder]');if(folderManage){openFolderManager(folderManage.dataset.manageFolder);return;}const manage=e.target.closest('button[data-manage-cat]');if(manage){openCategoryManager(manage.dataset.manageCat);return;}const folder=e.target.closest('button[data-folder]');if(folder){state.filterCategory=folder.dataset.folderCat;state.filterFolderId=folder.dataset.folder;renderAll();return;}const b=e.target.closest('button[data-cat]');if(!b)return;state.filterCategory=b.dataset.cat;state.filterFolderId='';renderAll();});
  ...
  $$('[data-close]').forEach(b=>b.addEventListener('click',()=>closeModal(b.dataset.close)));$$('.modal-wrap').forEach(w=>w.addEventListener('click',e=>{if(e.target===w)w.classList.remove('show');}));
