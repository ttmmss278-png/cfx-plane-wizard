# 冲击式水轮机工程工具箱

将常用 CFX 前后处理、批量求解、几何处理、数据管理与数值验证工具整合到统一入口，提供模块导航、搜索、帮助、刷新、独立打开和全屏操作。各工具以独立模块运行，尽量保持原有计算逻辑、文件处理和下载能力，同时通过 iframe 隔离不同模块的 CSS、DOM 与脚本。

## 功能模块

| 编号 | 模块 | 运行方式 |
| --- | --- | --- |
| 01 | CFX-Post 批量导出 | 浏览器 + 可选本地服务 |
| 02 | CFX 连跑 BAT 生成器 | 纯浏览器 |
| 03 | 截面二维归一化投影 | 纯浏览器 |
| 04 | CFX-Post 圆截面生成器 | 纯浏览器 |
| 05 | CFX 批量转 DEF | 本机 PowerShell 服务 + CFX-Pre |
| 06 | CFX-Post 公式与命令库 | 纯浏览器 + 可选 GitHub 私有数据同步 |
| 07 | 网格无关性验证 | 纯浏览器 |
| 08 | 射流质量分级评价与智能优选 | 纯浏览器 |
| 09 | 偏离圆度计算 | 纯浏览器 |

偏离圆度模块支持按喷嘴输入三维轴线两点、匹配任意数量的截面轮廓与面积，并绘制各喷嘴随 X/D 变化的曲线。曲线纵轴可切换百分比或无量纲 C，支持喷嘴勾选和 PNG / SVG 导出；结果数据可导出 Excel / CSV。

## 本地开发

需要 Node.js 20 或更高版本。

```bash
npm install
npm run dev
```

构建静态文件：

```bash
npm run build
npm run preview
```

## 使用全部功能

Windows 下双击根目录的 `启动工程工具箱.bat`。首次运行会自动安装依赖并构建，然后启动仅监听 `127.0.0.1` 的本地服务。

在线网页的“CFX-Post 批量导出”和“CFX 批量转 DEF”共用同一个 Windows 本地服务。新电脑首次使用时，在任一模块顶部下载 `Pelton-Toolbox-Local-Service-Windows.zip`，完整解压后双击 `安装并启动本地服务.bat`。安装程序会把运行文件复制到当前用户的 `%LOCALAPPDATA%\PeltonToolbox\LocalService`，不需要管理员权限、Node.js 或 npm；以后直接点击网页中的“启动本地服务”即可。浏览器首次调用时可能会询问是否打开 `pelton-toolbox` 链接或允许本地网络访问，请选择允许。发布包的版本、大小与 SHA256 记录在 [`local-service-manifest.json`](https://ttmmss278-png.github.io/cfx-plane-wizard/downloads/local-service-manifest.json)。

“CFX 批量转 DEF”需要：

1. Windows PowerShell；
2. 本机已安装 ANSYS CFX-Pre；
3. 保持本地服务的 PowerShell 窗口开启。

静态部署（包括 GitHub Pages）不能直接访问本机完整文件路径或调用 CFX-Pre，因此相关模块会显示统一的下载、安装和启动入口。

## 项目结构

```text
src/                         统一工具箱外壳与主题系统
public/modules/              9 个工程功能模块
public/downloads/            GitHub Pages 提供的 Windows 本地服务包
local-def-service/           批量导出与批量转 DEF 共用的本地服务
安装并启动本地服务.bat         下载包中的首次安装入口
启动本地服务.bat               已安装后的服务启动入口
启动工程工具箱.bat             Windows 一键启动入口
```

## 数据与隐私

工程文件均在浏览器或本机处理，不会自动上传。第 01 个“CFX-Post 批量导出”可选用本地服务读取用户本次选择的完整文件路径；服务未连接时退回普通浏览器导入，不恢复上次公共目录。第 06 个“CFX-Post 公式与命令库”只有在用户主动配置后，才会通过 GitHub API 向私有数据仓库同步公式、CCL、分类和文件夹数据；GitHub Token 默认只保存在当前浏览器会话中。第 07、08、09 个数值评价模块仅在浏览器本地处理输入与计算。第 09 个模块只将用户输入的喷嘴轴线坐标保存在当前浏览器本地。
